...
function breakDownCents(value) {
  let arr = value
    .split("")
    .slice(1)
    .map((n, i) => parseFloat(`.${"0".repeat(i)}${n}`));
  let tenths = makePlaces(arr[0], 0.1);
  let hundreths = makePlaces(arr[1], 0.01);
  let temp = {};
  temp.total = value;
  if (tenths.length >= 1) {
    temp.tenths = tenths;
...
function getChange(register, change) {
  ...
  if (cents.total > 0) {
    ...
    let dimes = register.getChange(0.1);
    let nickles = register.getChange(0.05);
    let pennies = register.getChange(0.01);
    delete cents.total;
